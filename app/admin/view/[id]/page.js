...
export default function AdminInfluencerView() {
  ...
  // Зареждаме инфо за инфлуенсъра от admin API
  useEffect(() => {
  ...
  const load = useCallback(async (days) => {
    if (!id) return
    setLoading(true)
    const res = await fetch(`/api/dashboard/orders?viewId=${id}&days=${days}`)
  ...
  useEffect(() => { if (id) load(0) }, [load, id])
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          ...
          <div className="card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 22, fontWeight: 700, color: 'var(--accent-dk)' }}>{commission}%</span>
            </div>
          </div>
        </div>
          ...
          <table style={{ minWidth: 860 }}>
            <thead>
              <tr>
                ...
                <th title={`${commission}% от пълната цена`}>Комисионна</th>
                <th>Статус</th>
              </tr>
            </thead>
            <tbody>

...
export async function syncCampaign(campaign, { sinceOverride = null } = {}) {
  ...
  if (orderIds.length) {
    ...
    // Провалена заявка ≠ „няма записани“ — иначе всички поръчки минават за нови и се разпращат мейли
    if (existErr) throw new Error(`existing orders lookup failed: ${existErr.message}`)

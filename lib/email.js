...
// Известие до админа при нов чат въпрос от инфлуенсър (или до инфлуенсъра при отговор от админ)
export async function sendNewChatMessage({ to, influencerName, senderRole, messagePreview, link }) {
  const fromAddr = process.env.EMAIL_FROM_ADDRESS
  const fromName = process.env.EMAIL_FROM_NAME || 'RealFood Influencer Portal'
  const url = link || PORTAL_URL
  const subject = senderRole === 'admin'
    ? '💬 Нов отговор от RealFood'
    : `💬 Нов въпрос от ${influencerName}`
  const heading = senderRole === 'admin' ? 'Имаш нов отговор' : `Нов въпрос от ${influencerName}`
  const preview = (messagePreview || '').slice(0, 300)
  const html = `<!DOCTYPE html><html lang="bg"><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:40px 16px;background:#f8f7f4;font-family:Helvetica,Arial,sans-serif;">
  <div style="max-width:560px;margin:auto;background:#fff;border-radius:16px;padding:32px;border:1px solid #e8e6e0;">
    <p style="font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:#6b6b68;margin:0 0 4px;">Чат</p>
    <h1 style="font-size:22px;color:#1a1a18;margin:0 0 18px;">${heading}</h1>
    <div style="background:#f8f7f4;border-radius:10px;padding:14px;margin-bottom:18px;font-size:14px;color:#1a1a18;white-space:pre-wrap;">${preview}</div>
    <table cellpadding="0" cellspacing="0" width="100%"><tr><td align="center">
      <a href="${url}" style="display:inline-block;background:#1D9E75;color:#fff;font-size:15px;font-weight:600;text-decoration:none;padding:12px 32px;border-radius:10px;">Отвори чата →</a>
    </td></tr></table>
  </div>
</body></html>`
  const client = getGraphClient()

...
// Еднакъв момент във времето, независимо от изписването на часовата зона.
...
// PostgREST реже всеки select на 1000 реда независимо от .limit(), затова
// четем на страници. buildQuery(from, to) връща заявка с .range().
export async function fetchAllRows(buildQuery, { pageSize = 1000, max = 20000 } = {}) {
  const rows = []
...
export async function refreshOrderStatuses(orders, { batchSize = 100 } = {}) {
  ...
  for (let i = 0; i < orders.length; i += batchSize) {
    ...
    for (const stored of batch) {
      ...
      // Сравняваме по стойност, не по низ: Shopify връща "…Z", а Postgres
      // "…+00:00" — иначе всяка анулирана поръчка би излизала „променена“.
      const cancelChanged = withCancelled && !sameInstant(cancelledAt, stored.cancelled_at)

 ...
 *
 * Ако Shopify не върне discount_allocations (стар формат или script discount),
 * третираме всички продукти в поръчката като комисионни — тъй като поръчката
 * вече е потвърдена като направена с промокода.
 */
function sanitizeOrder(order, promoCode) {
  ...
  const hasDiscountAllocations = promoAppIndex >= 0

  // Fallback спестявания от discount_codes ако нямаме allocation данни
  const fallbackSavings = hasDiscountAllocations ? 0 : parseFloat(
  ...
  const lineItems = (order.line_items || []).map(item => {
    ...
    if (hasDiscountAllocations) {
      // Точно засичане по discount_application_index
      ...
      // Shopify не е върнал allocation данни → всички продукти са комисионни
      isDiscounted = true
    }
  ...
  // Ако нямаме allocation данни, вземаме общото спестяване от discount_codes
  if (!hasDiscountAllocations && fallbackSavings > 0) {

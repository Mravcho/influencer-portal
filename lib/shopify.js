...
function sanitizeOrder(order, promoCode) {
  // Спестяването от промокода е винаги достъпно в discount_codes
  const codeEntry = (order.discount_codes || []).find(
    dc => dc.code?.toUpperCase() === promoCode.toUpperCase()
  )
  const savingsFromCode = parseFloat(codeEntry?.amount || 0)

  // Намираме индекса в discount_applications
  ...
  let lineItems = (order.line_items || []).map(item => {
    ...
    if (promoAppIndex >= 0) {
      // Точно засичане по discount_allocation
  ...
  // Fallback: ако discount_allocations не са върнали нито един match
  // (стар Shopify формат, Script discount, или discount върху целия order)
  // → третираме всички продукти като комисионни
  if (!anyDiscounted) {

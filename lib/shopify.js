...
// Анулира съществуваща Shopify Order. Нашето приложение я е създало → има право да я анулира.
// reason: 'customer' | 'fraud' | 'inventory' | 'declined' | 'other'
export async function cancelOrder(orderId, { reason = 'other', email = false, refund = false } = {}) {
  const params = new URLSearchParams()
  if (reason) params.set('reason', reason)
  if (email)  params.set('email', 'true')
  if (refund) params.set('refund', 'true')
  const res = await shopifyPost(`/orders/${orderId}/cancel.json?${params.toString()}`, {})
  return res.order
}

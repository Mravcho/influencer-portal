...
// Проверява кои от подадените кодове реално съществуват в Shopify.
// codeDiscountNodeByCode е case-insensitive и връща null, ако няма такъв код.
// Питаме на партиди с alias-и — 20 кода в една заявка вместо 20 заявки.
export async function checkDiscountCodesExist(codes) {
  const unique = [...new Set((codes || []).filter(Boolean))]
  const result = {}
  for (let i = 0; i < unique.length; i += 20) {
    const batch = unique.slice(i, i + 20)
    const query = 'query {\n' + batch.map((code, n) =>
      `  c${n}: codeDiscountNodeByCode(code: ${JSON.stringify(code)}) { id }`
    ).join('\n') + '\n}'
    try {
      const data = await shopifyGraphQL(query)
      batch.forEach((code, n) => { result[code] = !!data?.[`c${n}`] })
    } catch (err) {
      console.error('checkDiscountCodesExist batch failed:', err.message)
      // При грешка не твърдим, че кодът липсва — оставяме го непроверен.
      batch.forEach(code => { result[code] = null })
    }
  }
  return result
}
...
// Извлича image URL за списък от product IDs.
// Преди всички продукти се теглеха наведнъж (Promise.all) — при поръчки с много
// различни продукти Shopify връща 429 (REST лимит ~2 заявки/сек), грешката се
// гълташе и снимката се записваше като null, презаписвайки добрата.
// Сега: по 3 паралелно, до 4 опита при 429/5xx. Продукт без снимка → null;
// неуспешно теглене → липсва в резултата (извикващият запазва старата снимка).
export async function fetchProductImages(productIds) {
  ...
  async function fetchOne(pid) {
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        ...
        const status = Number((err.message.match(/Shopify API error (\d+)/) || [])[1])
        if (status === 404) { result[pid] = null; return } // изтрит продукт
        if (status === 429 || status >= 500) { await sleep(1000 * (attempt + 1)); continue }
        return
      }
    }
  }
  ...
  for (let i = 0; i < unique.length; i += CONCURRENCY) {
    await Promise.all(unique.slice(i, i + CONCURRENCY).map(fetchOne))
  }
  return result
}

// Една страница от ВСИЧКИ поръчки в магазина (не само с промокод) — за
// UTM статистиката, където поръчка без промокод също се брои.
// Подава се или `since` (първа страница), или `pageInfo` (следваща).
// Забележка: Shopify не приема `status`/`fields` заедно с page_info.
export async function fetchStoreOrdersPage({ since = null, pageInfo = null } = {}) {
  const params = pageInfo
    ? { limit: 250, page_info: pageInfo }
    : {
        limit: 250,
        status: 'any',
        fields: 'id,order_number,landing_site,created_at,total_price,currency,financial_status,cancelled_at',
        created_at_min: since,
      }
  const data = await shopifyFetch('/orders.json', params)
  return { orders: data.orders || [], nextPageInfo: data._nextPageInfo || null }
}

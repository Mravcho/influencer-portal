...
export async function syncInfluencer(influencer, {
  ...
  try {
    ...
    // Вече записаните снимки по заглавие — ако Shopify не върне снимка при този
    // sync (throttling, мрежова грешка), не презаписваме добрата с null.
    const knownImages = {}
    for (const r of existing) {
      for (const li of r.line_items || []) if (li.image_url) knownImages[li.title] = li.image_url
    }
    ...
    const rows = shopifyOrders.map(o => ({
      ...
      line_items:             (o.line_items || []).map(li => li.image_url ? li : { ...li, image_url: knownImages[li.title] || null }),
